...
const ShowProducts = () => {
  ...
  const fetchData = (search = "", pageNumber = 1) => {
    let apiUrl = `http://127.0.0.1:8000/customer/show_products/?page=${pageNumber}`;
    if (search) {
  ...
  const handleDeleteConfirm = () => {
    axios
      .delete(
        `http://127.0.0.1:8000/customer/deactivate_customer/?pk=${deleteCustomerId}`
      )
      .then((response) => {
  ...
  const handleInsertProductInOffer = () => {
    ...
    console.log(payload);
    axios
      .post(`http://127.0.0.1:8000/customer/insert_offer_details/`, payload)
      .then((response) => {
        setSuccessMessage(response.data.message);
        setOfferModel(false);
